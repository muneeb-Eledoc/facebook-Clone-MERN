...
export default function Conversation({ currentChat, chat, sentByMe, currentUser, arrivedMessage, setRecipient, setCurrentChat, setMessages, onlineUsers, onlineStatus }) {
    const [indiviualRecipient, setIndiviualRecipient] = useState({})
    const [lastMessage, setLastMessage] = useState({})

    useEffect(() => {
        const getMem = async () => {
            const res = await axios.get(`/users?userId=${chat?.members?.find(m => m !== currentUser._id)}`);
            setIndiviualRecipient(res.data)
        }
        getMem()
    }, [chat, currentUser, setRecipient, onlineUsers])
